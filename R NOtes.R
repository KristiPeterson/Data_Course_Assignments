...
abline(glm(data[,3] ~ data[,2])) #finding the linear line of the linear model, using baseR //all the built in functions
...
iris = read.csv("iris.csv") # this prints it directly to the screen. we want to save it to an object
?read.csv
getwd()
...
iris[iris$Species == "virginica",]   #[rows, columns] in the [] you are calling the vector needed
...
otus = read.csv("otu_table.csv", as.is = TRUE, stringsAsFactors = FALSE,
         check.names = FALSE)
metadata = read.csv("otu_mapping.csv", as.is = TRUE, stringsAsFactors = TRUE,
         check.names = FALSE)
...
# LOOK UP PACKAGES
# CRAM R 
# HOMEWORK ASSIGNMENT Which OTU is present in more sites than any other?? (This one is much tougher to answer)

dim(otus$setosa)

#want to know what is greater than zero
TF = otus >0
# True technically = 1 so if we add up all the rows, False will be 0 and True will be one
...
#USING PA
otus_pa =otus
identical(otus, otus_pa)  #check if they are identical otus and otus_pa
otu_pa[otu_pa >0] = 1
#######################
#      PACKAGES!      #
#######################
...
library(ggplot2) # library() loads a given package PUT THEM AT THE BEGINNING OF YOUR CODE
ggplot(iris, mapping = aes(x = Sepal.Length, y = Petal.Length, col = Species)) +
  geom_point() +
...
#everytime it sees the word "Snail" its going to write Green
